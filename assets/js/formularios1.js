...
function validarFormulario() {
    ...
    if (nombre.value == "") {
     ...
     // Validar con JavaScript que el campo “nombre” sólo acepte caracteres de letras y espacios en blanco.
        ...
        // Si no es válido mostrará el mensaje “El campo email sólo no acepta espacios en blanco”
        alert("El campo email esta incompleto ");
        // y se pondrá el foco en el campo “email”
        nombre.focus();
        valido = false;
    }
    else if (!expRegTfno.exec(tfno.value) ){
        alert(" Nº de telefono incorrecto");
        tfno.focus();
        valido = false;
    }
    
    else if (mensaje, length> 255){
        alert("El texto ha excedido el limite de caracteres");
        formulario.submit();
    }
    else if ( edad.value <18 || edad.value >120 ){
        alert ("La edad debe ser mayor a 18 y menor de 120")
